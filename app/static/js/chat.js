/* ==========================================================================
  Sidhant Patel AI — Chat Application with Image Support
   ========================================================================== */
...
(function ($) {
  ...
  function handleImageSelect(e) {
    ...
    if (file.size > 4 * 1024 * 1024) {
      showToast("Image too large. Max 4MB allowed.");
      return;
    }
  ...
  function addMessage(text, type, cache, imageUrl) {
    ...
    var imgTag = imageUrl
      ? '<img class="msg-img" src="' + imageUrl + '" alt="Shared image">'
      : "";
    ...
    if (cache && cache.cached_tokens > 0) {
      cacheTag =
        '<span class="cache-tag"><i class="bi bi-lightning-fill"></i>' +
        cache.hit_rate +
        "% cached</span>";
    }
    ...
    if (hasImage) {
      ...
      $.ajax({
        ...
        .done(function (r) {
          hideTyping();
          if (r.text) {
            addMessage(r.text, "bot", r.cache);
            playVoice(r.voice);
          }
        })
        .fail(function (x) {
          hideTyping();
          addMessage(getError(x), "bot");
        })
        .always(done);
    } else {
      $.ajax({
  ...
  function playVoice(src) {
    ...
    if ("Notification" in window && Notification.permission === "granted") {
      new Notification("Sidhant Patel AI", {
  ...
  function getError(xhr) {
    if (xhr.status === 400 && xhr.responseJSON)
      return xhr.responseJSON.error || "Bad request.";
  ...
  function sendAudio(blob) {
    ...
    $.ajax({
      ...
      .done(function (r) {
        hideTyping();
        if (r.transcription) addMessage(r.transcription, "user");
        if (r.text) {
          addMessage(r.text, "bot", r.cache);
          playVoice(r.voice);
        }
      ...
      .fail(function (x) {
        hideTyping();
        addMessage("Could not process audio.", "bot");
      })
      .always(function () {
        isProcessing = false;
        $sendBtn.prop("disabled", false);
  ...
  function showToast(msg) {
    $(".toast-msg").remove();
    var t = $(
      '<div class="toast-msg"><i class="bi bi-info-circle me-1" style="color:var(--primary)"></i>' +
        esc(msg) +
        "</div>",
    );
    $("body").append(t);
